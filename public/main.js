  ...
  if (results.multiHandLandmarks) {
    for (const landmarks of results.multiHandLandmarks) {
      ...
      drawConnectors(canvasCtx, landmarks, HAND_CONNECTIONS, { 
        color: '#00FF00', 
        lineWidth: Math.max(2, canvasElement.width / 320) // レスポンシブな線幅
      });
...
async function startCamera(facingMode = 'user') {
  try {
    ...
    // 既存のリソースをクリーンアップ
    await stopCamera(false); // UIは更新しない

    // カメラからの映像ストリームをMediaPipeに送る
    camera = new Camera(videoElement, {
      onFrame: async () => {
        if (isHandsReady && hands && isRunning) {
...
socket.on('connect', () => {
  ...
  dataChannel = peerConnection.createDataChannel('handData', {
    ordered: false, // 順序を保証しない（低遅延優先）
    maxRetransmits: 0 // 再送信しない（リアルタイム優先）
  });
